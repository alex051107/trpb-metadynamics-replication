...
> **核心数字**: FP-034 resid offset bug 揭示 coord-system 整体错 / λ 3.80 → 100.79 Å⁻² 与 Miguel 80 自洽（1.26×）/ pilot max_s=10.9 plateau，gate s>12 未达 / 5 月 1 号 kill-switch
...
## Slide 10 — Pilot 结果：corrected path 揭开真实 coordinate system
（约 2 分钟）

OK，最后上数据。这张 slide 之前版本我写成 "500× 加速"，后来做 adversarial self-review 时砍了——**500× 的说法 conflate 了两件事**：coordinate-system 换了 和 MetaD exploration 真的加速了。我 honest 重新 frame 下。

**核心对比**：**同一个 start.gro**（500 ns 经典 MD 终态），**在 naive path 上 s=1，在 corrected path 上 s=7**。信息量很大——我们那个 500 ns 经典 MD 的终态**根本从来不在 O basin**，它在 PC region 附近。过去五周我们以为 walker "卡在 O basin 出不去"，实际上 walker 从来没在 O basin 里——**只是被错误的 CV 误标注了**。老 run 7.7 ns max_s=1.75 这个数字从来不是"没在动"，是"CV 在新 coordinate 下 s 值恰好长这样"。

**当前 pilot 实际进展**（到讲 slide 时为止，sim 大约 1.4 ns，wall 1h51m）：**max s = 10.921 @ t = 437 ps**。之后**有将近 1 ns 的 plateau**——walker 没突破 s=11 这个次级 barrier。z 稳定在 1.5 Å² 左右，wall-hit fraction 远低于老 run。**这说明 CV 在响应**，也说明**可能有 intermediate barrier**，或者需要 10-walker 的 shared bias 才推得过去。

**Honest framing**：这个 slide 证明的是 CV 修好了、walker 能 access s=5-11 的新 region；它**不证明** walker 能一路跑到 s=15 (C-end)。Gate for 10-walker launch 是 max_s > 12 sustained；如果 pilot 结束前越不过，我们就直接起 10-walker 看 shared bias 能不能推。
