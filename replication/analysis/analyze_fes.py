#!/usr/bin/env python3
"""
Analyze a PLUMED/GROMACS free-energy surface for the TrpB Path-CV benchmark.

The script:
  - loads fes.dat (s, z, F) grids from plumed sum_hills
  - identifies O / PC / C basin minima using the JACS 2019 state windows
  - estimates projected barriers along the s coordinate
  - plots the 2D FES with basin annotations
  - compares key quantities to the JACS 2019 PfTrpS(Q2) reference values

The default reference comparison is intentionally narrow:
  - C relative to the lower of the O/PC minima: ~5 kcal/mol
  - PC -> C barrier: ~6 kcal/mol

Those are the explicit values recorded in the local JACS 2019 reading notes.
Use CLI flags to override them for other intermediates or systems.
"""
