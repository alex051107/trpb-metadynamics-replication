#!/bin/bash
#SBATCH --job-name=seqaligned_walkers
...
WALKER_DIR=/work/users/l/i/liualex/AnimaLab/metadynamics/miguel_2026-04-23/seqaligned_walkers/walker_${WALKER_ID}

cd ${WALKER_DIR}

echo "=== walker_${WALKER_ID} start === $(date)"
echo "PLUMED_KERNEL=${PLUMED_KERNEL}"
echo "host=$(hostname)  gpu=$(nvidia-smi -L | head -1)"
echo "WALKERS_ID in plumed.dat:"
grep "WALKERS_ID" plumed.dat
