#!/usr/bin/env python3
"""
Convert an AMBER topology/restart pair into GROMACS topology/coordinate files.

This script is designed for the PfTrpS(Ain) benchmark system described in
JACS 2019 and the local replication workflow:

  AMBER conventional MD  ->  ParmEd conversion  ->  GROMACS + PLUMED MetaD

Hard requirements from the Runner brief:
  - print intermediate values (atom count, box lengths, total charge)
  - assert atom count, box consistency, and charge consistency
  - stop loudly if ParmEd cannot serialize the custom GAFF/ff14SB system
  - perform an AMBER vs GROMACS single-point energy comparison on the same
    last-frame coordinates, with relative deviation < 0.1%

This script can be run on Longleaf after:
