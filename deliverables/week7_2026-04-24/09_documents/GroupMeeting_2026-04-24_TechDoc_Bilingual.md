...
| MetaD parameters | Miguel primary (HEIGHT=0.15, BF=10, DIFF, UNITS A/kcal) |
...
| Sim length | 8 ns  (40 193 COLVAR rows) |
| Seed | `start.gro` = terminal frame of 500 ns Ain classical MD |

### 4.2 s(t) exploration / s(t) 探索

**EN** — Under identical start.gro, Miguel params, and single-walker setup, the only variable between baseline 45324928 and pilot 45515869 is `path.pdb`. Observed behaviour:
...
**中文** — 同一个 start.gro、同一套 Miguel 参数、同样 single walker 的前提下, baseline 45324928 和 pilot 45515869 的**唯一变量是 `path.pdb`**。观测:
...
**EN** — `reports/figures/sz_2d_distribution.png`, generated by `reports/figures/plot_sz_distribution.py`, renders the FES `F(s, z) = -kT ln( p(s, z) / p_max )` at T = 350 K (kT = 0.696 kcal/mol), clipped at 6 kcal/mol. Panel (a) is single-walker baseline 45324928 on the naive path; panel (b) is single-walker pilot 45515869 on the seq-aligned path. Both panels use the identical KDE grid and colorscale for direct comparison. Panel (a) shows density confined to a narrow `s < 2` ridge; panel (b) shows density spread across `s ∈ [1, 12]`. The red star in panel (b) marks start.gro at (s = 7.09, z = 1.68).
...
**中文** — `reports/figures/sz_2d_distribution.png`, 由 `reports/figures/plot_sz_distribution.py` 生成, 绘制 FES `F(s, z) = -kT ln( p(s, z) / p_max )` (T = 350 K, kT = 0.696 kcal/mol, clip 在 6 kcal/mol)。panel (a) 是 single-walker baseline 45324928 on naive path; panel (b) 是 single-walker pilot 45515869 on seq-aligned path。两个 panel 用相同 KDE grid 和相同色标, 直接对比。panel (a) 密度被压在窄窄的 `s < 2` 条带上; panel (b) 密度铺开到 `s ∈ [1, 12]`。panel (b) 里红色星标是 start.gro 的位置 (s = 7.09, z = 1.68)。
