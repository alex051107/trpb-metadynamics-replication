...
这一节回答的问题是：本文反复出现的 `MD/MetaD` 术语到底分别是什么意思。目标不是穷举定义，而是给你一个能读懂仓库和论文的最小工作词汇表。

| 术语 | 中文解释 | 在本项目中的具体含义 |
|---|---|---|
| `Ain` | internal aldimine | `PLP` 与 `Lys82` 形成 Schiff base 的静息中间体 |
| `Aex1` | external aldimine | 外部底物接管与 `PLP` 的共价连接后形成的早期中间体 |
| `A-A` | aminoacrylate | 脱水后的活化中间体，是 indole 进攻前的重要状态 |
| `Q2` | quinonoid 2 | JACS 2019 中用于闭合态自由能景观分析的关键中间体 |
| `COMM domain` | communication domain | TrpB 中承担大尺度开合的结构区段 |
| `FES` | free energy surface | 在 `CV` 空间上重建的自由能景观 |
| `CV` | collective variable | 用少数几个坐标概括体系慢变量 |
| `Path CV` | 路径集体变量 | 用 `s(R)` 与 `z(R)` 表示沿路径前进程度与偏离程度 |
| `WT-MetaD` | Well-Tempered MetaDynamics | Gaussian hills 高度逐步 temper 的 MetaDynamics 变体 |
| `RESP` | Restrained Electrostatic Potential | 基于 `QM ESP` 拟合部分电荷的方法 |
| `GAFF` | General AMBER Force Field | 为小分子/非标准残基提供通用参数的 AMBER 力场 |
| `ff14SB` | AMBER protein force field | 本项目蛋白部分的标准参数集 |
| `TIP3P` | 三位点水模型 | 论文与当前复刻使用的显式水模型 |
